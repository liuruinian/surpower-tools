...
@Data
@ApiModel("驾驶证识别请求参数")
public class OcrDriverLicenseParam implements Serializable {
     ...
     * true：返回发证机关
     * false：不返回发证机关
     * 如果无该参数，系统默认不返回发证机关。如果输入参数不是Boolean类型，则会报非法参数错误AIS.0101。
     */
    @ApiModelProperty(name = "returnIssuingAuthority", value = "returnIssuingAuthority")
    @JsonProperty(value = "return_issuing_authority")
    @JSONField(name = "return_issuing_authority")
